...
def init():
    ...
    for contract in ADDRESSES:
        CONTRACTS[contract] = W3.eth.contract(
            address=ADDRESSES[contract], abi=ABIS[contract]
        )
...
def get_token_meta(token):
    ...
    w3_multicall.add(W3Multicall.Call(normalize_address(token), "symbol()(string)"))
    w3_multicall.add(W3Multicall.Call(normalize_address(token), "decimals()(uint8)"))
...
def get_middleware(net):
    return (
        CONTRACTS["middleware_service"]
        .functions.middleware(normalize_address(net))
        .call()
    )
...
def get_nets():
    ...
    for i in range(total_entities):
        w3_multicall.add(
            W3Multicall.Call(ADDRESSES["net_registry"], "entity(uint256)(address)", i)
        )
    ...
    for net in nets:
        w3_multicall.add(
            W3Multicall.Call(
                ADDRESSES["middleware_service"], "middleware(address)(address)", net
            )
        )
    ...
    for i in range(len(nets)):
        results.append({"net": nets[i], "middleware": middlewares[i]})
    return results
...
def get_ops():
    ...
    for i in range(total_entities):
        w3_multicall.add(
            W3Multicall.Call(ADDRESSES["op_registry"], "entity(uint256)(address)", i)
        )
...
def get_op_nets(operator):
    ...
    for i in range(len(optins)):
        if optins[i]:
            results.append({"net": nets[i]["net"], "middleware": nets[i]["middleware"]})
    return results
...
def get_vaults():
    ...
    for i in range(total_entities):
        w3_multicall.add(
            W3Multicall.Call(ADDRESSES["vault_factory"], "entity(uint256)(address)", i)
        )
    ...
    for v_idx in range(len(results)):
        w3_multicall.add(
            W3Multicall.Call(results[v_idx]["delegator"], "TYPE()(uint64)")
        )
        rev_result_idxs.append((v_idx, 0))
        if results[v_idx]["slasher"] != "0x0000000000000000000000000000000000000000":
            w3_multicall.add(
                W3Multicall.Call(results[v_idx]["slasher"], "TYPE()(uint64)")
            )
...
def vaults():
    ...
    for vault in vaults:
        ...
        print_i4(
            f'Collateral: {vault["collateral"]} ({get_token_meta(vault["collateral"])["symbol"]})'
        )
        print_i4(
            f'Delegator: {vault["delegator"]} ({DELEGATOR_TYPES[vault["delegator_type"]]})'
        )
        print_i4(
            f'Slasher: {vault["slasher"]} ({SLASHER_TYPES[vault["slasher_type"]]})\n'
        )
...
def netstakes(address):
    ...
    for op in opsvaults:
        ...
        for collateral in collaterals.keys():
            ...
            for vault in collaterals[collateral]:
                ...
                print_i8(
                    f'Type: {DELEGATOR_TYPES[vault["delegator_type"]]} / {SLASHER_TYPES[vault["slasher_type"]]}'
                )
    ...
    for collateral in total_stakes.keys():
        token_meta = get_token_meta(collateral)
        print_i2(
            f'Collateral {collateral} ({token_meta["symbol"]}): {total_stakes[collateral] / 10**token_meta["decimals"]}'
        )
